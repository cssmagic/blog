...
**接口** 是模块化软件设计的一个基本工具。接口约定了一个完整模块所能实现的功能。举例来说，JavaScript 应用程序有一个常见问题，一旦互联网连接断开之后，应用程序就停止工作了。为了解决这个问题，你可能会使用本地存储（localStorage）特性，并且定期向服务器同步数据变化。不幸的是，部分浏览器并不支持本地存储，所你可能不得不降级到 Cookie 或 Flash 的存储功能（这取决于你需要保存的数据量）。
...
当你需要为相同的接口编写多个实现时，在代码中使用规范的注释格式来清晰地描述出接口的用途，是一种良好的实践。编写可以自我描述的代码是很重要的。当你需要创建一个新的具体实例时，只需要简单地调用一个预定义的工厂函数并传入你需要覆盖的方法就可以了。（译法：本节提到的“实现”指的是对接口的实现，我们可以为相同的接口编写不同的实现。）
...
这里有一个重要部分是数据流（stream）接口。首先你创建了工厂函数（在这个例子中使用了 `O.js`，当然也可以是其它函数，只要它返回的对象可以用来演示接口）。这个例子中只有一个方法：`.save()`
...
&nbsp;
> * [参与评论](https://github.com/cssmagic/blog/issues/32)
